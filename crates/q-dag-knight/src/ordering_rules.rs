...
impl OrderingEngine {
    ...
    pub async fn process_vertex(
        ...
        debug!(
            "Processing vertex {} for ordering in round {}",
        ...
        // Check cache first
        {
            ...
            if let Some(cached_ordering) = cache.get(&vertex.round) {
                ...
                stats.cache_hit_rate =
                    (stats.cache_hit_rate * stats.total_orderings_computed as f64 + 1.0)
        ...
        // Update statistics
        {
            ...
            stats.average_ordering_time_ms = (stats.average_ordering_time_ms
                * (stats.total_orderings_computed - 1) as f64
                + processing_time_ms as f64)
                / stats.total_orderings_computed as f64;
        }
    ...
    async fn add_vertex_to_graph(&self, vertex: &Vertex) -> Result<()> {
        ...
        debug!(
            "Added vertex {} with {} causal dependencies",
            hex::encode(vertex.id),
            dependencies.len()
        );
    ...
    pub async fn verify_ordering(&self, round: Round, ordering: &[VertexId]) -> Result<bool> {
        ...
        for &vertex_id in ordering {
            if let Some(dependencies) = graph.get(&vertex_id) {
                for &dep_id in dependencies {
                    if let (Some(&vertex_pos), Some(&dep_pos)) =
                        (position.get(&vertex_id), position.get(&dep_id))
                    {
                        if dep_pos >= vertex_pos {
                            warn!(
                                "Ordering violation: dependency {} comes after vertex {}",
                                hex::encode(dep_id),
    ...
    pub async fn cleanup_cache(&self, keep_rounds: u64) {
        ...
        debug!(
            "Cleaned up ordering cache, keeping rounds >= {}",
            cutoff_round
        );
    }
