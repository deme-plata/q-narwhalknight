...
use crate::{Priority, TransactionType};
use q_types::NodeId;
...
impl FairnessAnalyzer {
    ...
    pub async fn record_enqueue(
        ...
        node_activity.avg_priority =
            node_activity.total_priority as f64 / node_activity.enqueues as f64;
        ...
        debug!(
            "Recorded enqueue: node={}, type={:?}, priority={}",
            hex::encode(&node_id),
            tx_type,
            priority
        );
    ...
    pub async fn record_dequeue(
        &mut self,
        tx_type: TransactionType,
        timestamp: Instant,
    ) -> Result<()> {
        // Find corresponding enqueue to calculate wait time
        let wait_time = if let Some(enqueue_record) = self
            .enqueue_history
            .iter()
        ...
        debug!(
            "Recorded dequeue: type={:?}, wait_time={}ms",
            tx_type,
            wait_time.as_millis()
        );
    ...
    fn calculate_type_fairness(&self) -> Result<HashMap<TransactionType, f64>> {
        ...
        for tx_type in tx_types {
            if let Some(activity) = self.type_activity.get(&tx_type) {
                let enqueue_ratio =
                    activity.enqueues as f64 / self.enqueue_history.len().max(1) as f64;
                let dequeue_ratio =
                    activity.dequeues as f64 / self.dequeue_history.len().max(1) as f64;
    ...
    fn calculate_wait_time_statistics(&self) -> Result<(f64, f64)> {
        ...
        let variance =
            wait_times.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / wait_times.len() as f64;
...
mod tests {
    ...
    async fn test_enqueue_recording() {
        ...
        analyzer
            .record_enqueue(node_id, TransactionType::User, 100)
            .await
            .unwrap();
    ...
    async fn test_fairness_metrics() {
        ...
        analyzer
            ...
            .unwrap();
        analyzer
            .record_dequeue(TransactionType::User, Instant::now())
            .await
            .unwrap();
