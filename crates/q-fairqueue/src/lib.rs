...
pub use bandwidth_allocation::{AllocationStrategy, BandwidthAllocator};
...
impl QuantumFairQueue {
    ...
    pub async fn new(node_id: NodeId, phase: Phase, config: QueueConfig) -> Result<Self> {
        ...
        let scheduler =
            QuantumScheduler::new(config.clone(), quantum_rng.as_ref(), lattice_vrf.as_ref())
                .await?;
    ...
    pub async fn enqueue(
        ...
        // Check for censorship attempts
        {
            ...
            if anti_censorship
                .is_censorship_attempt(&tx_id, from_node)
                .await?
            {
                warn!(
                ...
                anti_censorship
                    .apply_countermeasures(&tx_id, from_node)
                    .await?;
        ...
        let priority = self
            .calculate_priority(tx_type, from_node, enqueue_time)
            .await?;
        ...
        // Enqueue with calculated priority
        {
            ...
            if let Some(queue) = queues.get_mut(&tx_type) {
                ...
                trace!(
                    "Transaction {} enqueued with priority {}",
                    hex::encode(&tx_id),
                    priority
                );
            } else {
        ...
        // Update fairness metrics
        {
            ...
            analyzer
                .record_enqueue(from_node, tx_type, priority)
                .await?;
        }
    ...
    async fn quantum_eviction(
        &self,
        queue: &mut PriorityQueue<TransactionId, Priority>,
        tx_type: TransactionType,
    ) -> Result<()> {
        ...
        if let Some(ref qrng) = self.quantum_rng {
            let random_bytes = qrng.generate_bytes(4).await?;
            let random_index = u32::from_be_bytes([
                random_bytes[0],
                random_bytes[1],
                random_bytes[2],
                random_bytes[3],
    ...
    pub async fn update_bandwidth_allocation(
        &self,
        allocations: HashMap<NodeId, u64>,
    ) -> Result<()> {
...
mod tests {
    ...
    async fn test_quantum_fair_queue_creation() {
        ...
        let queue = QuantumFairQueue::new(node_id, Phase::Phase0, config)
            .await
            .unwrap();
    ...
    async fn test_enqueue_dequeue() {
        ...
        // Enqueue transaction
        queue
            .enqueue(tx_id, TransactionType::User, node_id)
            .await
            .unwrap();
    ...
    async fn test_priority_ordering() {
        ...
        queue
            .enqueue(user_tx, TransactionType::User, node_id)
            .await
            .unwrap();
        queue
            .enqueue(consensus_tx, TransactionType::Consensus, node_id)
            .await
            .unwrap();
        queue
            .enqueue(emergency_tx, TransactionType::Emergency, node_id)
            .await
            .unwrap();
