...
impl AntiCensorshipManager {
    ...
    pub async fn is_censorship_attempt(
        &mut self,
        tx_id: &TransactionId,
        from_node: NodeId,
    ) -> Result<bool> {
        ...
        if is_suspicious {
            warn!(
                "Potential censorship detected: tx={}, node={}",
                hex::encode(tx_id),
    ...
    pub async fn apply_countermeasures(
        &mut self,
        tx_id: &TransactionId,
        target_node: NodeId,
    ) -> Result<()> {
        if !matches!(
            ...
            CensorshipResistance::Active | CensorshipResistance::QuantumEnhanced
        ) {
            return Ok(());
        }

        info!(
            "Applying anti-censorship countermeasures for transaction {}",
            hex::encode(tx_id)
        );
    ...
    async fn analyze_censorship_patterns(
        &mut self,
        tx_id: &TransactionId,
        from_node: NodeId,
    ) -> Result<bool> {
        ...
        let pattern =
            self.rejection_patterns
                .entry(from_node)
                .or_insert_with(|| RejectionPattern {
                    total_rejections: 0,
        ...
        pattern
            .recent_rejections
            .retain(|&time| now.duration_since(time) < Duration::from_secs(300)); // 5 minute window
        ...
        if pattern.rejection_rate > 0.1 {
            // More than 0.1 rejections per second
            ...
            if !pattern
                .suspicious_patterns
                .iter()
                .any(|p| matches!(p, SuspiciousPattern::HighRejectionRate { .. }))
            {
    ...
    async fn get_most_suspicious_nodes(&self) -> Result<Vec<(NodeId, f64)>> {
        let mut suspicious: Vec<(NodeId, f64)> = self
            .rejection_patterns
            .iter()
            .map(|(&node_id, pattern)| {
...
mod tests {
    ...
    async fn test_censorship_detection() {
        ...
        for _ in 0..5 {
            let is_censorship = manager
                .is_censorship_attempt(&tx_id, node_id)
                .await
                .unwrap();
    ...
    async fn test_countermeasures() {
        ...
        manager
            .apply_countermeasures(&tx_id, node_id)
            .await
            .unwrap();
