...
use crate::parameters::{LatticeConfig, SecurityLevel};
...
impl LatticeKey {
    ...
    pub fn evaluate_vrf(
        ...
        for i in 0..params.dimension {
            let start = (i * 4) % 32;
            let bytes: [u8; 4] = hashed_randomness[start..start + 4]
                .try_into()
        ...
        Ok(VRFEvaluation::new(trapdoor_result, input.dimension))
    }

    /// Public evaluation for verification
    pub fn public_evaluate(
        &self,
        input: &LatticeSample,
        params: &LatticeParameters,
    ) -> Result<VRFEvaluation> {
        ...
        Ok(VRFEvaluation::new(reduced_eval, input.dimension))
    }

    /// Apply lattice trapdoor function
    fn apply_trapdoor_function(
        &self,
        input: &DVector<i64>,
        params: &LatticeParameters,
    ) -> Result<DVector<i64>> {
    ...
    fn test_key_serialization() {
        ...
        rt.block_on(async {
            ...
            assert_eq!(
                ...
                reconstructed_key.to_bytes().unwrap()
            );
        });
    }
