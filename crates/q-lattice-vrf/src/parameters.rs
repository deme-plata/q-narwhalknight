    ...
    fn generate_prime_modulus(bits: usize) -> BigInt {
        ...
        match bits {
            ...
            4096 => {
                ...
                BigInt::parse_bytes(prime_str.as_bytes(), 10)
                    .unwrap_or(BigInt::from(2u64.pow(31) - 1))
            }
...
impl LatticeConfig {
    ...
    pub fn for_security_level(level: SecurityLevel) -> Self {
        match level {
            ...
            SecurityLevel::Custom(bits) => Self::custom_config(bits),
        }
    }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if let Some(degree) = self.ring_degree {
            ...
            if self.optimizations.use_ntt && !degree.is_power_of_two() {
                return Err(anyhow!(
                    "Ring degree must be power of 2 for NTT optimization"
                ));
            }
        }
    ...
    pub fn estimate_performance(&self) -> PerformanceEstimate {
        ...
        let parallel_factor = if self.optimizations.parallel_processing {
            0.7
        } else {
            1.0
        };
        ...
        PerformanceEstimate {
            ...
            memory_usage_kb: (self.dimension * 8 + self.modulus.bits() as usize / 8) as u64,
        }
    }
...
impl ParameterRegistry {
    ...
    fn register_standard_configs(&mut self) {
        ...
        let _ = self.register(
            "dilithium-like".to_string(),
            ParameterSets::dilithium_like(),
        );
