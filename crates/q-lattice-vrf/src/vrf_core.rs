...
use crate::lattice::{LatticeKey, LatticeParameters, LatticeSample};
...
impl VRFOutput {
    ...
    pub fn from_hex(hex_str: &str) -> Result<Self> {
        let output = hex::decode(hex_str).map_err(|e| anyhow!("Invalid hex string: {}", e))?;
        Ok(Self::new(output))
    }
    ...
    pub fn extract_bits(&self, num_bits: usize) -> Result<Vec<bool>> {
        ...
        if num_bits > max_bits {
            return Err(anyhow!(
                "Cannot extract {} bits from {} byte output",
                num_bits,
                self.length
            ));
        }
        ...
        match self.proof_system {
            ProofSystem::Bulletproofs => {
                ...
                Ok(ProofComponents {
                    ...
                    challenge: if self.length > 32 {
                        Some(self.proof_data[32..].to_vec())
                    } else {
                        None
                    },
                    response: Vec::new(),
            ...
            ProofSystem::LatticeZK => {
                ...
                Ok(ProofComponents {
                    ...
                    response: if self.length > 64 {
                        self.proof_data[64..].to_vec()
                    } else {
                        Vec::new()
                    },
                })
...
impl VRFEvaluation {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let dimension = usize::from_be_bytes(
            bytes[0..8]
                .try_into()
                .map_err(|_| anyhow!("Invalid dimension bytes"))?,
        );

        let quality = f64::from_be_bytes(
            bytes[8..16]
                .try_into()
                .map_err(|_| anyhow!("Invalid quality bytes"))?,
        );

        let hash_len = usize::from_be_bytes(
            bytes[16..24]
                .try_into()
                .map_err(|_| anyhow!("Invalid hash length bytes"))?,
        );
        ...
        for i in 0..dimension {
            ...
            let value = i64::from_be_bytes(
                bytes[start..start + 8]
                    .try_into()
                    .map_err(|_| anyhow!("Invalid vector bytes"))?,
            );
...
mod tests {
    ...
    fn test_vrf_evaluation_serialization() {
        ...
        assert_eq!(
            ...
            reconstructed.evaluation_vector
        );
    }
