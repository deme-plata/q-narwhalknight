...
pub fn route(path: &str, config: &StaticConfig) -> RouteResult {
    ...
    if has_static_extension(clean) {
        ...
        if candidate.is_file() && is_safe_path(&candidate, root) {
            let cache = if {
                let l = clean.to_ascii_lowercase();
                l.ends_with(".html") || l.ends_with(".htm")
            } {
                // HTML entrypoints reference content-hashed bundles, so they must
                // never be frozen in the browser cache — otherwise a fresh deploy
                // is invisible until the 24h TTL expires. "no-cache" still allows
                // a conditional GET (etag → 304 when unchanged), so it's cheap but
                // always serves the latest dashboard on reload.
                "no-cache"
            } else if is_hashed_asset(clean) {
                "no-cache"
            } else {
                "no-cache"
            };
