...
pub struct QuantumNetwork {
    ...
    node_id: NodeId,
    current_phase: Phase,
    /// L-VRF for quantum-resistant peer selection
    vrf: Option<LatticeVRF>,
    /// Quantum RNG for connection nonces
...
impl QuantumNetwork {
    ...
    pub async fn new_with_phase(node_id: NodeId, phase: Phase) -> Result<Self> {
        ...
        info!("🌐 Initializing Q-Network {:?} with PeerId: {}", phase, peer_id);
        ...
        let qrng = if matches!(phase, Phase::Phase2 | Phase::Phase3 | Phase::Phase4) {
            info!("🌌 Initializing quantum RNG for connection nonces");
            ...
            match QuantumRNG::new(phase, config).await {
                Ok(qrng) => {
                    info!("✅ Quantum RNG initialized for network layer");
                    Some(qrng)
                }
                Err(e) => {
                    warn!("⚠️ Failed to initialize network QRNG: {}", e);
                    None
                }
            }
        } else {
