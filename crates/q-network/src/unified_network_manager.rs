...
impl UnifiedNetworkManager {
    ...
    pub async fn new(
        ...
        Ok(Self {
            ...
            // 2026-06-04: serving permits are now env-tunable via Q_BLOCK_PACK_PERMITS.
            // BLOCK_PACK_BASE_PERMITS=128 (default, unchanged for compat) lets up to 128
            // concurrent block-pack serves run long RocksDB forward-seeks on spawn_blocking;
            // under heavy requester load this exhausts the tokio blocking pool and STARVES
            // the block-production commit (also spawn_blocking) → PROD-WATCHDOG "FROZEN"
            // deadlock on the sole tip node (Delta, 2026-06-04). Operators on a producing
            // tip should cap this low (e.g. Q_BLOCK_PACK_PERMITS=8) to protect production.
            block_pack_semaphore: Arc::new(tokio::sync::Semaphore::new(
                std::env::var("Q_BLOCK_PACK_PERMITS")
                    .ok()
                    .and_then(|s| s.parse::<usize>().ok())
                    .filter(|&n| n > 0)
                    .unwrap_or(BLOCK_PACK_BASE_PERMITS),
            )), // v9.1.8 + adaptive: base, always-on permits (OOM-safe during sync)
                            ...
                            Message::Response { request_id, response } => {
                                ...
                                if !response.blocks.is_empty() {
                                    if let Some(ref tx) = self.block_sync_tx {
                                        ...
                                        // v10.11.43 SWARM-WEDGE FIX: NEVER .await this send on the swarm
                                        // event loop. handle_behaviour_event() runs INLINE on the
                                        // select_next_some() loop (called at line ~3514 with &mut self,
                                        // so it cannot be spawned). The bounded(4) block_sync channel's
                                        // consumer is the Ed25519+RocksDB applier ("seconds per batch").
                                        // When the node is far behind and the applier saturates, the
                                        // channel stays full; `tx.send().await` (v10.11.21) then BLOCKS
                                        // the swarm loop, so select_next_some() is never polled and NO
                                        // new ConnectionEstablished/Noise upgrades are serviced → kernel
                                        // accepts TCP but libp2p never completes → Broken pipe/reset →
                                        // 0 peers → broadcast NoPeersSubscribedToTopic → sends + browser
                                        // P2P dead (gdb: swarm future suspended, runtime idle, CPU 0%).
                                        // try_send keeps memory bounded (the v10.11.21 leak fix) WITHOUT
                                        // ever blocking the loop; on Full we drop and the peer re-serves
                                        // the batch on the next retry-driven sync round.
                                        match tx.try_send(response.blocks.clone()) {
                                            Ok(()) => {
                                                info!("✅ [BLOCK-PACK] Forwarded {} blocks to consensus for validation", n_blocks);
                                            }
                                            Err(tokio::sync::mpsc::error::TrySendError::Full(_)) => {
                                                warn!("⚠️ [BLOCK-PACK] applier busy (sync channel full) — dropping {} blocks; peer re-serves next sync round. Swarm stays responsive (no wedge).", n_blocks);
                                            }
                                            Err(tokio::sync::mpsc::error::TrySendError::Closed(_)) => {
                                                error!("❌ [BLOCK-PACK] consensus sync channel closed — blocks not forwarded");
                                            }
                                        }
                                    } else {
