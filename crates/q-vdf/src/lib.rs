...
impl QuantumVDF {
    ...
    pub async fn new(parameters: VDFParameters, phase: Phase) -> Result<Self> {
        info!(
            "Initializing Quantum VDF for {:?} with {} iterations",
    ...
    pub async fn evaluate(&self, input: &[u8], round: Round) -> Result<VDFOutput> {
        ...
        let vdf_result = self
            .compute_vdf(&seed, self.parameters.time_parameter)
            .await?;
    ...
    pub async fn verify(&self, input: &[u8], output: &VDFOutput) -> Result<bool> {
        ...
        let verification_result = self
            .verifier
            .verify(&seed, &output.output, &output.proof, output.iterations)
            .await?;
        ...
        info!(
            "VDF verification completed in {}ms: {}",
    ...
    async fn compute_vdf(&self, input: &[u8], iterations: u64) -> Result<Vec<u8>> {
        match self.parameters.vdf_type {
            VDFType::Wesolowski => self.compute_wesolowski_vdf(input, iterations).await,
            VDFType::Pietrzak => self.compute_pietrzak_vdf(input, iterations).await,
            VDFType::QuantumHybrid => self.compute_quantum_hybrid_vdf(input, iterations).await,
        }
    }
    ...
    async fn compute_quantum_hybrid_vdf(&self, input: &[u8], iterations: u64) -> Result<Vec<u8>> {
        ...
        self.compute_wesolowski_vdf(&enhanced_input, iterations)
            .await
    }
