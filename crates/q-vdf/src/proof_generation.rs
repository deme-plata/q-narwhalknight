use crate::{ProofType, VDFParameters, VDFProof};
    ...
    pub async fn generate_proof(
        ...
        let (proof_type, proof_data, aux_data) = match self.strategy {
            ProofStrategy::QuantumEnhanced => {
                self.generate_quantum_proof(input, output, iterations, vrf_result)
                    .await?
            }
            _ => {
                self.generate_classical_proof(input, output, iterations)
                    .await?
            }
        };
