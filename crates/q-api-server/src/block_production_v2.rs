...
async fn production_loop(
    ...
    loop {
        ...
        if iter % 30 == 0 {
            let h = app.node_status.read().await.current_height;
        ...
        let cur_h = app.node_status.read().await.current_height;
        let net_h = app.highest_network_height.load(Ordering::SeqCst);
        if net_h > 0 {
            let gap = net_h.saturating_sub(cur_h);
            if gap > 1000 { if iter % 30 == 0 { warn!("🚫 Production DISABLED: {} behind", gap); } continue; }
        ...
        for (pid, blk) in &blocks {
            ...
            match time::timeout(Duration::from_secs(30), app.storage_engine.save_qblock(blk)).await {
                Ok(Ok(_)) => debug!("✅ Block saved: h={}, p={}", blk.header.height, pid),
