...
impl BlockProducer {
    ...
    /// v10.11.49: read this process's anonymous RSS (GB) from /proc/self/status.
    /// Used purely for production backpressure; returns None off-Linux / on parse failure.
    fn current_anon_rss_gb() -> Option<f64> {
    ...
    /// v10.11.49: true when process anon-RSS is over the production ceiling.
    /// The in-memory DAG/consensus working set can accumulate faster than the periodic
    /// cleanup prunes it during sustained solo production, which previously spiraled to
    /// ~57 GB → OOM-restart loop. When over the ceiling we pause production so cleanup can
    /// catch up. Deletes NO data and never touches balances/blocks (those persist to RocksDB
    /// independently) — it only paces production. Tunable via Q_MAX_PRODUCE_RSS_GB (default 20).
    fn memory_backpressure_active() -> bool {
        if let Some(anon_gb) = Self::current_anon_rss_gb() {
            let max_gb: f64 = std::env::var("Q_MAX_PRODUCE_RSS_GB")
                .ok()
                .and_then(|s| s.parse().ok())
                .unwrap_or(20.0);
            if anon_gb > max_gb {
                warn!(
                    "🛑 [MEM-BACKPRESSURE] anon-RSS {:.1}GB > {:.1}GB ceiling — pausing block \
                     production so DAG cleanup catches up (no data touched, balances/blocks safe)",
                    anon_gb, max_gb
                );
                return true;
            }
    ...
    pub fn should_produce_block(&self) -> bool {
        // v10.11.49: memory backpressure — don't produce while anon-RSS is over the ceiling.
    ...
    pub async fn produce_block(&mut self) -> Option<QBlock> {
        ...
        // v10.11.49: memory backpressure chokepoint — refuse to produce while anon-RSS is
        // over the ceiling, so the in-memory DAG can't outrun cleanup into an OOM-restart loop.
        if Self::memory_backpressure_active() {
            return None;
        }
