...
pub async fn deploy_verify(
    ...
    // Auth is enforced by the AEGIS-QL `verify_founder_signature` middleware
    // layered on this route in main.rs. By the time execution reaches here the
    // founder signature + timestamp have been verified; the old in-handler
    // `is_master_wallet` string-match (spoofable via the public founder
    // address) has been removed. `headers` retained for the audit trail below.
    let _ = &headers;
...
pub async fn deploy_promote(
    ...
    // Auth enforced by the AEGIS-QL `verify_founder_signature` middleware in
    // main.rs (was a spoofable `is_master_wallet` string-match on the public
    // founder address — see SECURITY note on the route registration).
    let _ = &headers;
...
pub async fn admin_update_toggle(
    ...
    // Auth enforced by the AEGIS-QL `verify_founder_signature` middleware
    // layered on this route in main.rs. The old in-handler string-match against
    // the PUBLIC founder address (spoofable by anyone who knew it) is removed.
    // Actor for the audit trail — X-Wallet-Address was verified by the middleware.
    let actor = headers
        .get("X-Wallet-Address")
        .and_then(|v| v.to_str().ok())
        .unwrap_or("founder")
