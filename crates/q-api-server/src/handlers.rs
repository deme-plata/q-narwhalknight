...
pub async fn bootstrap_peers(
    ...
    // v10.11.44 STALE-HEIGHT FIX: /api/v1/status (this handler) was reporting
    // upgrade_manager.height(), which is only advanced when the upgrade manager
    // processes blocks and was stuck at the restart value (e.g. 18,532,739) while
    // the chain was really ~11k blocks higher. Wallets/explorers read this height,
    // so settled state looked frozen. Use the live current_height_atomic (the same
    // value the block-producer/mining paths update via fetch_max). Upgrade
    // activation still uses upgrade_manager.is_active() below — independent of this.
    let current_height = state.current_height_atomic.load(std::sync::atomic::Ordering::Relaxed);
...
pub async fn proof_tip(
    ...
    // Phase 2 (v10.9.41): live chain-wide recursive lattice tip proof.
    //
    // The producer-hook task in main.rs extends `state.lattice_tip_proof` on
    // every contiguous-height advance, so this handler is constant-time —
    // single RwLock::read() + serialize. The proof covers the full chain
    // from genesis (anchor=0, zero-state) to the current tip, in 176 bytes.
    //
    // Soundness: BLAKE3 + Ajtai-SIS commitment, no curve/pairing math, PQ.
    // Verifier <10ms (0.3ms on xeon-gold) — see q_recursive_proofs::tip_verify.
    let live_proof = state.lattice_tip_proof.read().await.clone();
    ...
    let (proof_version, proof_bytes, step_count, anchor_height, anchor_state_hex) = match live_proof {
        ...
        None => {
            // Producer-hook hasn't initialised yet (first few seconds of boot).
            ("placeholder-v0".to_string(), vec![0u8; 32], 0u64, 0u64, "0x".to_string() + &"0".repeat(64))
        }
    };
    ...
    if proof_version == "tip-blake3-fs-v1" {
        body["verifier_advice"] = serde_json::json!({
            "scheme": "BLAKE3 Fiat-Shamir hash-chain with public-input-bound commitment, chain-wide",
            "verify_target_ms": 10,
            "anchor_height": anchor_height,
            "anchor_state_hex": anchor_state_hex,
            "step_count": step_count,
            "note": "renamed from 'lattice-tip-v1' after DeepSeek crypto review (2026-05-16) — the actual reduction is BLAKE3 preimage resistance (128 bit Grover-quantum), not a lattice problem. Real lattice (Module-SIS) commitment deferred to v10.10.0.",
            "binding_fix_notes": "v10.9.41 commit() folds (anchor_height, anchor_state, tip_height, folded_state, transcript) into the keyed hash — closes the DeepSeek §0 anchor-swap forgery the pre-fix construction allowed.",
        });
    } else {
        body["verifier_advice"] = serde_json::json!({
            "warning": "Producer-hook task still warming up. Proof will be live within ~5 seconds of node start.",
...
pub async fn import_wallet(
    ...
    // 2026-05-17 SECURITY FIX (brainwallet incident): validate BIP39 BEFORE
    // hashing. The legacy SHA3-256(mnemonic) derivation accepts ANY string —
    // common phrases like "test test test..." or 12 made-up 3-letter words
    // deterministically derived to PUBLIC well-known addresses that multiple
    // users shared. Frontend now validates too, but the server is the last
    // line of defense if any non-web client (custom script, other wallet) hits
    // this endpoint.
    let normalized_mnemonic: String = mnemonic.trim().to_lowercase()
        .split_whitespace().collect::<Vec<&str>>().join(" ");
    if let Err(e) = bip39::Mnemonic::parse_in(bip39::Language::English, &normalized_mnemonic) {
        warn!("🚨 [SECURITY] Rejecting non-BIP39 mnemonic in import_wallet: {}", e);
        return Ok(Json(ApiResponse::error(
            "Invalid recovery phrase: must be a valid BIP39 mnemonic (wordlist + checksum). \
             Random words or numbers will not work — use a phrase generated by a BIP39-compliant wallet.".to_string()
        )));
    }
...
pub async fn sign_transaction(
    ...
    let tx_request = serde_json::json!({
        "wallet_id": wallet_id,
        "to": request.to,
        // v10.11.60: Amount/fee are u128; serialize as strings to avoid serde_json
        // "number out of range" panic on values > u64::MAX (see handlers.rs:6171).
    ...
    if let Some(status) = state.tx_status.get(&tx_hash) {
        ...
        let details = match status.value() {
            ...
            TxStatus::InMempool => {
                ...
                if still_in_pool {
                    build_privacy_response(tx_hash_str.clone(), "in_mempool".to_string(), None, 0, None)
                } else if let Ok(Some(stored_tx)) = state.storage_engine.load_transaction(&tx_hash).await {
                    // v10.11.44 GHOST-DROP FIX: tx_status here is stuck at InMempool —
                    // it's never promoted to Confirmed when the producer packs the tx
                    // into a block. So once the tx leaves the mempool (packed), this
                    // branch falsely reported "dropped" for a COMMITTED tx, which made
                    // wallets show settled transfers (and mining rewards) as reversed.
                    // A tx is only persisted once it lands in a committed block ⇒ if
                    // load_transaction finds it, it's confirmed, not dropped.
                    let ts = Some(stored_tx.timestamp.timestamp() as u64);
                    ...
                    build_privacy_response(tx_hash_str.clone(), "dropped".to_string(), None, 0, None)
                }
            }
...
pub async fn send_transaction_signed(
    ...
    let (tx_type, token_type, tx_data) = if token_str_upper == "QUG" || token_str_upper == "NATIVE-QUG" {
        ...
        // Custom token: accept EITHER a symbol (CHONK, PACI, DOGE2 …) OR a
        // qnk-/0x- contract address. v10.11.52 fix: the frontend Send page (and
        // any symbol-based caller) posts token_type as the ticker, but this
        // handler previously only ran parse_wallet_address(), which rejected
        // symbols with "Address must start with 0x or qnk". resolve_token_address()
        // parses addresses directly and looks up symbols in deployed_contracts
        // (plus QUGUSD / bridge / index-fund tokens), matching the /transactions/send
        // and /dex/swap paths so custom-token transfers work uniformly.
        let token_addr = match resolve_token_address(&state, &request.token_type).await {
            Ok(a) => a,
            Err(e) => return Ok(Json(ApiResponse::error(format!(
                "Invalid token_type (must be QUG, QUGUSD, a known token symbol, or a qnk-prefixed contract address): {}", e
            )))),
        };
    ...
    let dashboard_txs: Vec<serde_json::Value> = recent_txs
        ...
        .map(|tx| {
            ...
            serde_json::json!({
                ...
                // v10.11.60: Amount is u128; serde_json (no arbitrary_precision) PANICS
                // serializing u128 > u64::MAX ("number out of range"), which the json!
                // macro .unwrap()s → aborts the whole node. Emit as a string instead.
...
pub async fn get_wallet_transaction_history(
    ...
    // 1. Load regular transactions for the wallet.
    // v10.11.45: read the dedicated TRANSFERS index first (sent + received, with NO
    // coinbase flood), then top up with a few recent mining rewards so heavy-mining
    // wallets still see their actual transfers instead of 100 entries all buried under
    // coinbase. Purely read-only (no writes, no state change).
    let limit = 100usize;
    let mut combined_txs: Vec<q_types::Transaction> = Vec::new();
    {
        let mut combined_seen: std::collections::HashSet<[u8; 32]> = std::collections::HashSet::new();
        match state.storage_engine.load_transfers_for_wallet(&wallet_bytes, limit).await {
            Ok(transfers) => {
                for tx in transfers {
                    if combined_seen.insert(tx.id) { combined_txs.push(tx); }
                }
            }
            Err(e) => warn!("Failed to load transfers for wallet: {}", e),
        }
        if let Ok(mining) = state.storage_engine.load_transactions_for_wallet(&wallet_bytes, 25usize).await {
            for tx in mining {
                if combined_seen.insert(tx.id) { combined_txs.push(tx); }
            }
        }
    ...
    info!("📜 [v10.11.45] Loaded {} regular tx entries (transfers + recent mining) for wallet", unified_history.len());
...
pub async fn get_node_config(
    ...
    // FIX: was hardcoded to beta 185.182.185.227 (ABANDONED FORK) — new nodes cached an
    // unreachable addr (beta-IP paired with epsilon peer-id) and never synced. Default to
    // epsilon (live chain), env-overridable.
