...
impl QuantumTradingAnalytics {
    ...
    async fn start_quantum_collection_tasks(&self) -> Result<()> {
        ...
        tokio::spawn(async move {
            // v10.11.51 OOM-LEAK FIX: build the interval ONCE before the loop. It was
            // previously recreated every iteration, and a tokio interval's FIRST tick()
            // fires immediately — so this was a busy-loop pushing millions of data-points
            // per second, all stamped with the current time, so retain() (which keeps points
            // within the retention window) never pruned them. market_history grew ~1.4 GB/sec
            // until OOM (confirmed via jemalloc heap profiling). Creating the interval once
            // makes tick() actually wait collection_interval_seconds between samples.
            let collection_interval = {
                let config_guard = config.read().await;
                config_guard.collection_interval_seconds
            };
            let mut interval = tokio::time::interval(std::time::Duration::from_secs(
                collection_interval.max(1),
            ));
            loop {
                interval.tick().await;
