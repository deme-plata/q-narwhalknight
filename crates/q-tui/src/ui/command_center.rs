//! Command Center: Network visualization view for the node TUI.
//!
...
//!   [R] Radar full-screen  [O] Ocean full-screen
//!   [V] Topology view      [Esc] Overview (default, all panels)
...
pub enum Panel {
    ...
    OceanFull,  // [O] - ocean takes full screen
    Topology,   // [V] - topology graph view
}
...
impl CommandCenterState {
    ...
    /// Advance animations by one tick. Reads metrics from the App.
    ...
    pub fn handle_key(&mut self, key: char) -> bool {
        let new = match key {
            ...
            'r' | 'R' => Panel::Overview,
            'o' | 'O' if self.focused_panel != Panel::OceanFull => Panel::OceanFull,
            'o' | 'O' => Panel::Overview,
            'v' | 'V' if self.focused_panel != Panel::Topology => Panel::Topology,
            'v' | 'V' => Panel::Overview,
            _ => return false,
        };
...
pub fn draw_command_center(f: &mut Frame, area: Rect, app: &App) {
...
fn draw_swarm_status(f: &mut Frame, area: Rect, app: &App) {
    let metrics = app.metrics.read().unwrap();

    let peer_count = metrics.peer_count as u32;
    let connected = metrics.peer_count > 0;
    let blocks_mined = metrics.blocks_mined;
    let active_miners = metrics.active_miners;
    let bytes_down = metrics.total_bytes_in;
    let bytes_up = metrics.total_bytes_out;
    let block_height = metrics.block_height;
    let network_height = metrics.network_height;
    ...
    let mut lines: Vec<Line> = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{}", block_height), Style::default().fg(Color::Cyan)),
            Span::styled(" / ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{}", network_height), Style::default().fg(Color::White)),
            Span::styled("   Miners: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", active_miners), Style::default().fg(Color::Yellow)),
            Span::styled("   Mined: ", Style::default().fg(Color::Gray)),
            Span::styled(format!("{}", blocks_mined), Style::default().fg(Color::Yellow)),
        ]),
        Line::from(vec![
...
fn draw_topology(f: &mut Frame, area: Rect, app: &App) {
    let pc = app.metrics.read().map(|m| m.peer_count as u32).unwrap_or(0);
    ...
    let tick = app.command_center.radar.tick;
...
fn draw_footer(f: &mut Frame, area: Rect) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled("[O]", c),  Span::styled("cean  ", d),
        Span::styled("[V]", c),  Span::styled(" Topology  ", d),
        Span::styled("[Esc]", c), Span::styled(" Overview  ", d),
        Span::styled("[Tab]", c), Span::styled(" Next view", d),
    ]);
