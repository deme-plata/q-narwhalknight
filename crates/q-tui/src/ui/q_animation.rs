...
pub struct QAnimation {
    ...
    seed: u64,
    /// When true, animation is completely disabled (--no-animation / Q_NO_ANIMATION=1)
    disabled: bool,
}

impl QAnimation {
    pub fn new() -> Self {
        let disabled = std::env::var("Q_NO_ANIMATION")
        ...
        // Startup splash: trigger immediately on boot (unless disabled)
        if !disabled {
            anim.phase = Phase::Converge;
        }

        anim
    }

    /// Create with animation explicitly disabled (for --no-animation flag)
    pub fn new_disabled() -> Self {
        Self {
            phase: Phase::Idle,
            frame: 0,
            idle_ticks: 0,
            trigger_interval: 720,
            seed: 0x517E_CA11_4E49_6874,
            disabled: true,
        }
    }
...
mod tests {
    ...
    fn test_animation_lifecycle() {
        let mut anim = QAnimation::new_disabled();
        assert!(!anim.is_active());

        // Enable and trigger manually
        anim.disabled = false;
        anim.trigger();
        assert!(anim.is_active());
    ...
    #[test]
    fn test_startup_splash() {
        // QAnimation::new() auto-triggers startup splash (unless Q_NO_ANIMATION is set)
        let anim = QAnimation::new_disabled();
        assert!(!anim.is_active()); // disabled = no splash
    }
