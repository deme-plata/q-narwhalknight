...
impl DedicatedCircuitManager {
    ...
    pub async fn new(config: TorConfig, node_id: NodeId) -> Result<Self> {
        info!(
            "🔧 Initializing DedicatedCircuitManager for validator {}",
        ...
        let qos_manager = Arc::new(Mutex::new(AdaptiveQoS::new(
            config.latency_target_ms.unwrap_or(300),
        )));
    ...
    async fn create_circuit_for_purpose(
        &self,
    ...
    pub async fn get_best_circuit_for_peer(
        &self,
        peer_onion: &str,
        purpose: CircuitPurpose,
    ) -> Result<u64> {
        ...
        if let Some(purpose_circuits) = circuits.get_mut(&purpose) {
            ...
            if let Some(circuit) = purpose_circuits
                .iter_mut()
                .find(|c| c.peer_assignments.contains(&peer_onion.to_string()))
            {
            ...
            if let Some(circuit) = purpose_circuits.iter_mut().min_by(|a, b| {
                a.utilization_score()
        ...
        anyhow::bail!(
            "No circuits available for peer {} with purpose {:?}",
            peer_onion,
            purpose
        );
    }
        ...
        for (purpose, purpose_circuits) in circuits.iter_mut() {
            ...
            for (index, circuit) in purpose_circuits.iter().enumerate() {
                if circuit.needs_rotation() {
                    debug!(
                        "🔄 Rotating {:?} circuit {} (age: {}s)",
                        purpose,
                        circuit.id,
                        circuit.created_at.elapsed().as_secs()
                    );
    ...
    pub async fn get_circuit_stats(&self) -> HashMap<CircuitPurpose, CircuitPurposeStats> {
        ...
        for (purpose, purpose_circuits) in circuits.iter() {
            ...
            let latencies: Vec<u64> = purpose_circuits
                .iter()
            ...
            let purpose_stats = CircuitPurposeStats {
                ...
                active_connections: purpose_circuits
                    .iter()
