...
impl P2PMessageHandler {
    ...
    async fn handle_transaction(&self, data: Vec<u8>, hash: Bytes32, timestamp: u64) -> Result<(), MessageError> {
        if data.is_empty() {
    ...
    async fn handle_block(&self, data: Vec<u8>, hash: Bytes32, height: u64, timestamp: u64) -> Result<(), MessageError> {
        if data.is_empty() {
    ...
    async fn handle_consensus(&self, consensus_type: ConsensusType, data: Vec<u8>, timestamp: u64) -> Result<(), MessageError> {
        if data.is_empty() {
