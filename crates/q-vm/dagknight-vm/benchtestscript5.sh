...
# Check if multi-coin-test exists
if [ ! -f "./target/release/multi-coin-test" ]; then
    echo -e "${GREEN}Building multi-coin-test...${NC}"
    cargo build --release --bin multi-coin-test
    if [ $? -ne 0 ]; then
...
run_test() {
    ...
    timeout ${TIMEOUT} ./target/release/multi-coin-test \
        --nodes ${nodes} \
    ...
    for i in $(seq 0 $((nodes-1))); do
        ...
        if [ ! -z "$node_tps" ]; then
            node_tps_sum=$(echo "$node_tps_sum + $node_tps" | bc)
    ...
    if [ $node_tps_count -gt 0 ]; then
        avg_node_tps=$(echo "scale=2; $node_tps_sum / $node_tps_count" | bc)
    fi
    ...
    # Copy the detailed results file
