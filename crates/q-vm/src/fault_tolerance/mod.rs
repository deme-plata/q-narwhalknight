...
impl RecoveryManager {
    ...
    pub async fn execute_with_recovery<T, E, F>(&self, tasks: Vec<F>) -> Result<Vec<T>>
    where
        T: Send + 'static,
        ...
        T: Send + 'static,
    {
        ...
        let mut failed_indices: Vec<usize> = results
            .iter()
            .enumerate()
        ...
        info!(
            "Found {} failed tasks, will retry up to {} times",
            failed_indices.len(),
            max_retries
        );
    ...
    pub async fn record_node_failure(&self, node_id: &str) {
        ...
        if new_reliability < 0.3 {
            ...
            warn!(
                "Node {} marked as failed (reliability: {})",
                node_id, new_reliability
            );
        }
    }
    ...
    pub async fn get_healthiest_nodes(&self, count: usize) -> Vec<String> {
        ...
        let mut nodes: Vec<_> = reliability
            .iter()
