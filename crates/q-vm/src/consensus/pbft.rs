...
impl PbftConsensus {
        ...
        tokio::spawn(async move {
            loop {
                ...
                if elapsed > view_change_timeout {
                    ...
                    for peer in &peers {
                        let _ = tx_network
                            .send((peer.clone(), PbftMessage::ViewChange(view_change.clone())))
                            .await;
                    }
    ...
    async fn handle_prepare_request(&self, req: PrepareRequest) {
        ...
        self.prepare_requests
            .insert((req.view, req.seq_num), req.clone());
        ...
        let _ = self
            .tx_network
            .send((
                req.primary_id.clone(),
                PbftMessage::PrepareResponse(prepare_response),
            ))
            .await;
    }
        ...
        if entry.len() >= self.get_quorum_size() {
            ...
            for peer in &self.peers {
                let _ = self
                    ...
                    .send((
                        peer.clone(),
                        PbftMessage::CommitRequest(commit_request.clone()),
                    ))
                    .await;
            }
        }
    ...
    async fn handle_commit_request(&self, req: CommitRequest) {
        ...
        for peer in &self.peers {
            let _ = self
                ...
                .send((
                    peer.clone(),
                    PbftMessage::CommitResponse(commit_response.clone()),
                ))
                .await;
        }
    }
    ...
    async fn handle_view_change(&self, vc: ViewChange) {
        ...
        // Store view change
        self.view_changes
            .insert((vc.new_view, vc.node_id.clone()), vc.clone());
            ...
            *self.is_primary.write().await = is_primary;
            ...
            if is_primary {
                ...
                for peer in &self.peers {
                    let _ = self
                        .tx_network
                        .send((peer.clone(), PbftMessage::NewView(new_view.clone())))
                        .await;
                }
            }
    ...
    pub async fn propose_block(
        ...
        parent_hash: BlockHash,
        transactions: Vec<Transaction>,
    ) -> Result<BlockHash, VmError> {
        let is_primary = *self.is_primary.read().await;
        if !is_primary {
            return Err(VmError::ConsensusFailure(
                "Not the primary node".to_string(),
            ));
        }
        ...
        let prepare_request = PrepareRequest {
            ...
            block_data: bincode::serialize(&block)
                .map_err(|_| VmError::SerializationError("Serialization failed".to_string()))?,
        ...
        for peer in &self.peers {
            let _ = self
                ...
                .send((
                    peer.clone(),
                    PbftMessage::PrepareRequest(prepare_request.clone()),
                ))
                .await;
        }
...
impl ConsensusEngine for PbftConsensus {
    ...
    async fn broadcast_contract(&self, hash: [u8; 32], bytecode: Vec<u8>) -> Result<(), VmError> {
        ...
        match self.propose_block(parent_hash, vec![tx]).await {
            Ok(_) => Ok(()),
            Err(e) => Err(VMError::ConsensusFailure(format!(
