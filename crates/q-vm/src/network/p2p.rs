...
impl P2PMessageHandler {
    ...
    pub async fn handle_message(&self, message: NetworkMessage) -> Result<(), MessageError> {
        match message {
            ...
            NetworkMessage::Consensus {
                ...
                self.handle_consensus(_consensus_type, data, _timestamp)
                    .await
            }
            NetworkMessage::ComputeTask {
                ...
                self.handle_compute_task(_contract, model, input, _timestamp)
                    .await
            }
            NetworkMessage::ComputeResult {
                _contract,
                output,
                _proof,
                _resources,
                ...
                self.handle_compute_result(_contract, output, _proof, _resources, _timestamp)
                    .await
            }
            NetworkMessage::NodeStatus {
                _node_id,
                _status,
                _available_resources,
                _timestamp,
            } => {
                self.handle_node_status(_node_id, _status, _available_resources, _timestamp)
                    .await
            }
            NetworkMessage::ModelRegistry {
                _action,
                data,
                _timestamp,
    ...
    async fn handle_consensus(
        ...
        if data.is_empty() {
            return Err(MessageError::InvalidMessage(
                "Empty consensus data".to_string(),
            ));
        }
    ...
    async fn handle_model_registry(
        ...
        if data.is_empty() {
            return Err(MessageError::InvalidMessage(
                "Empty model registry data".to_string(),
            ));
        }
