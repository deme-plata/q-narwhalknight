...
impl ModelCache {
    ...
    pub fn new(provider: CacheProvider, memory_size: usize, redis_url: Option<String>) -> Self {
        // Create memory cache
        let memory_size =
            NonZeroUsize::new(memory_size).unwrap_or(NonZeroUsize::new(10000).unwrap());
        ...
        if matches!(provider, CacheProvider::Redis | CacheProvider::Layered)
            && redis_client.is_none()
        {
    ...
    async fn check_redis_cache(
        ...
        let key = format!(
            "dagknight:model:{}:{}",
            model,
            hex::encode(Self::hash_bytes(input))
        );
    ...
    pub async fn set(&self, model: &str, input: &[u8], output: &[u8], ttl: u64) {
        ...
        if matches!(self.provider, CacheProvider::Redis | CacheProvider::Layered) {
            if let Some(client) = &self.redis_client {
                self.update_redis_cache(client, model, input, output, ttl)
                    .await;
            }
        }
    }
        ...
        match client.get_async_connection().await {
            Ok(mut conn) => {
                let _: Result<(), redis::RedisError> =
                    conn.set_ex(&key, output, ttl as usize).await;
            }
            Err(e) => {
