...
/// v10.11.45: Wallet TRANSFER index — same key shape as CF_WALLET_TX_INDEX but ONLY
/// for non-coinbase txns (transfers/token-transfers/swaps the wallet sent or received).
/// Separate CF so a heavy-mining wallet's coinbase rewards (one per block) don't flood
/// the history and bury actual transfers. Additive + locally rebuildable (NOT state-synced).
pub const CF_WALLET_TRANSFER_INDEX: &str = "cf_wallet_transfer_index";
...
impl QStorage {
    ...
    pub async fn save_transaction(&self, tx: &q_types::Transaction) -> Result<()> {
        ...
        // v10.11.45: also write the transfers-only index (skip coinbase/mining rewards)
        let is_coinbase = matches!(tx.tx_type, q_types::TransactionType::Coinbase);
        if !is_coinbase {
            self.hot_db.put(CF_WALLET_TRANSFER_INDEX, &sender_key, &tx.id).await?;
        }
        ...
        if tx.from != tx.to {
            ...
            if !is_coinbase {
                self.hot_db.put(CF_WALLET_TRANSFER_INDEX, &recipient_key, &tx.id).await?;
            }
        }
    ...
    pub async fn save_transactions(&self, transactions: &[q_types::Transaction]) -> Result<()> {
        ...
        for tx in transactions {
            ...
            let sender_key = Self::build_wallet_tx_key(&tx.from, tx.timestamp.timestamp(), &tx.id);
            batch_ops.push((CF_WALLET_TX_INDEX, sender_key.clone(), tx.id.to_vec()));
            // v10.11.45: transfers-only index (skip coinbase so transfers aren't buried)
            let is_coinbase = matches!(tx.tx_type, q_types::TransactionType::Coinbase);
            if !is_coinbase {
                batch_ops.push((CF_WALLET_TRANSFER_INDEX, sender_key, tx.id.to_vec()));
            }

            // v3.5.8-beta: Index by recipient wallet (if different)
            if tx.from != tx.to {
                let recipient_key = Self::build_wallet_tx_key(&tx.to, tx.timestamp.timestamp(), &tx.id);
                batch_ops.push((CF_WALLET_TX_INDEX, recipient_key.clone(), tx.id.to_vec()));
                if !is_coinbase {
                    batch_ops.push((CF_WALLET_TRANSFER_INDEX, recipient_key, tx.id.to_vec()));
                }
            }
        }
    ...
    /// v10.11.45: Load TRANSFERS (non-coinbase) for a wallet — sent OR received — newest
    /// first, via the dedicated transfer index. Mirrors load_transactions_for_wallet but
    /// scans CF_WALLET_TRANSFER_INDEX, so mining rewards never crowd out real transfers.
    /// Read-only. Returns whatever is indexed (grows for new txns; old txns appear after backfill).
    pub async fn load_transfers_for_wallet(
        &self,
        wallet: &[u8; 32],
        limit: usize,
    ) -> Result<Vec<q_types::Transaction>> {
        ...
        // 1) Primary: the dedicated transfers index (fast; populated for all new txns).
        match self.hot_db.scan_prefix(CF_WALLET_TRANSFER_INDEX, &prefix).await {
            Ok(entries) => {
                for (_key, tx_id_data) in entries {
                    if transactions.len() >= limit {
                        break;
            ...
            Err(e) => {
                warn!("Failed to scan wallet transfer index: {}", e);
            }
        }
        // v10.11.47: lazy-on-read backfill removed — it was too slow for heavy-mining
        // wallets (re-scanned every call, never reached transfers buried under tens of
        // thousands of coinbase). Old transfers are populated by the one-time throttled
        // background backfill (backfill_transfer_index_from_blocks), env-gated in main.rs.

        Ok(transactions)
    }

    /// v10.11.47: One-time, throttled, READ-ONLY backfill of the transfers index from
    /// committed blocks. Iterates [from_height, to_height], and for every NON-coinbase tx
    /// writes the (sender, recipient) transfer-index entries — exactly what save_transactions
    /// now does for new blocks. Bounded memory (one block at a time), throttled (sleeps every
    /// few hundred blocks) so it can run on a live mainnet node without starving production.
    /// Idempotent: re-writing the same index keys is harmless. Writes ONLY the additive
    /// transfers index — never balances, blocks, or any authoritative state.
    pub async fn backfill_transfer_index_from_blocks(
        &self,
        from_height: u64,
        to_height: u64,
    ) -> Result<(u64, u64)> {
        let mut blocks_scanned: u64 = 0;
        let mut transfers_indexed: u64 = 0;
        let mut h = from_height;
        while h <= to_height {
            // v10.11.48: use get_qblock_any_format (searches qblock:height: AND qblock:dag:
            // key formats) — get_qblock_by_height returns None for the ~545K DAG-format
            // blocks, so the v47 backfill skipped almost everything (found only 1 transfer).
            if let Ok(Some(block)) = self.get_qblock_any_format(h).await {
                let mut batch: Vec<(&str, Vec<u8>, Vec<u8>)> = Vec::new();
                for tx in &block.transactions {
                    if matches!(tx.tx_type, q_types::TransactionType::Coinbase) {
                        continue;
                    }
                    let ts = tx.timestamp.timestamp();
                    let sender_key = Self::build_wallet_tx_key(&tx.from, ts, &tx.id);
                    batch.push((CF_WALLET_TRANSFER_INDEX, sender_key, tx.id.to_vec()));
                    if tx.from != tx.to {
                        let recipient_key = Self::build_wallet_tx_key(&tx.to, ts, &tx.id);
                        batch.push((CF_WALLET_TRANSFER_INDEX, recipient_key, tx.id.to_vec()));
                    }
                    transfers_indexed += 1;
                }
                if !batch.is_empty() {
                    if let Err(e) = self.hot_db.write_batch(batch).await {
                        warn!("[TRANSFER-BACKFILL] write_batch failed at height {}: {}", h, e);
                    }
                }
            }
            blocks_scanned += 1;
            // Throttle: yield + brief sleep every 250 blocks so production/serving keep priority.
            if blocks_scanned % 250 == 0 {
                tokio::time::sleep(std::time::Duration::from_millis(40)).await;
                if blocks_scanned % 10_000 == 0 {
                    info!("[TRANSFER-BACKFILL] progress: {} blocks scanned, {} transfers indexed (at height {})",
                        blocks_scanned, transfers_indexed, h);
                }
            }
            h += 1;
        }
        info!("[TRANSFER-BACKFILL] ✅ complete: {} blocks scanned, {} transfers indexed ({}..={})",
            blocks_scanned, transfers_indexed, from_height, to_height);
        Ok((blocks_scanned, transfers_indexed))
    }
    ...
    // v10.9.55 (2026-05-18): `safe_batched_convergence_v103` REMOVED.
    //
    // The v1.0.3 convergence migration deleted every `wallet_balance_*` key from
    // RocksDB and rebuilt balances by replaying `qblock:height:1..tip`. On a chain
    // whose canonical-layer keys are sparse (4.65M gaps proven via direct ldb scan
    // 2026-05-18 — see docs/technical-review-sparse-chain-truth-v1.md), the replay
    // silently skipped missing heights and produced wallet totals missing all the
    // mining rewards from those gaps.
    //
    // On Epsilon — where 100K Contabo VPSes mined for ~3 months at 8 GH/s collective,
    // earning the authoritative wallet balances — running this migration would have
    // destroyed real PoW-backed value. Violates CLAUDE.md "BALANCE INTEGRITY" Rule 1
    // (max-wins) and Rule 3 (Epsilon balances are authoritative).
    //
    // The divergence-check error message in `state_sync_api.rs` that previously
    // recommended "delete the flag and restart" has been updated to remove that
    // dangerous advice.
    //
    // Audit deferred to v10.9.56: 9 other `delete_by_prefix(b"wallet_balance_")`
    // call sites in this file + main.rs need either an `is_genesis_node()` guard
    // or similar tombstone-removal treatment.
    //
    // The `migration_safe_convergence_v103_done` flag stays unset forever. The
    // QUGUSD ghost-prevention gates in `state_sync_api.rs:980-988` and `:1597`
    // that check this flag are now permanently in conservative mode (reject QUGUSD
    // updates from peers post-bootstrap), which is the safer outcome.
