...
impl RocksDBKV {
    ...
    pub async fn open_hot_db_with_phase<P: AsRef<Path>>(path: P, phase: Phase) -> Result<Self> {
        ...
        let cfs = vec![
            ...
            Self::create_wallet_transfer_index_cf(&block_cache), // v10.11.45: transfers-only index (no coinbase flood)
    ...
    /// v10.11.45: Wallet TRANSFER index (non-coinbase only) — same options/prefix as the
    /// tx index. Auto-created via CF migration on existing mainnet DBs (purely additive).
    fn create_wallet_transfer_index_cf(cache: &rocksdb::Cache) -> ColumnFamilyDescriptor {
        let mut opts = Options::default();
        opts.set_compression_type(rocksdb::DBCompressionType::Lz4);
        opts.set_write_buffer_size(Self::scale_write_buffer(32)); // 32MB
        opts.set_max_write_buffer_number(2);
        opts.set_prefix_extractor(rocksdb::SliceTransform::create_fixed_prefix(32));
        Self::apply_shared_block_cache(&mut opts, cache);
        ColumnFamilyDescriptor::new(crate::CF_WALLET_TRANSFER_INDEX, opts)
    }
...
impl KVStore for RocksDBKV {
    ...
    async fn scan_prefix(&self, cf: &str, prefix: &[u8]) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        // 2026-06-05 CRITICAL: was iterating RocksDB synchronously on the tokio worker thread.
        // Under concurrent load (node_status / wallet-history / mining handlers all scanning),
        // every worker blocked in DBIter::Next, starving the runtime → the production task could
        // never run → node "froze" every ~20 min. Same class the write path fixed in v0.9.94 but
        // the scan path was missed. Move the blocking iteration to the dedicated blocking pool.
        let db = self.db.clone();
        let cf = cf.to_string();
        ...
        .await
        .map_err(|e| anyhow::anyhow!("scan_prefix blocking task failed: {}", e))?
    }

    async fn scan_prefix_seek(&self, cf: &str, prefix: &[u8], limit: usize) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        // 2026-06-05 CRITICAL: same runtime-starvation bug as scan_prefix — synchronous RocksDB
        // raw-iterator scan was blocking the tokio worker thread. Move to the blocking pool.
        let db = self.db.clone();
        let cf = cf.to_string();
        let prefix = prefix.to_vec();
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // Use raw iterator with seek — does NOT consult bloom filters (ldb-scan semantics).
    ...
    async fn scan_all(&self, cf: &str) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        // 2026-06-05 CRITICAL: full-CF synchronous scan was blocking the tokio worker thread —
        // same runtime-starvation class as scan_prefix. Move to the blocking pool.
        let db = self.db.clone();
        let cf = cf.to_string();
        tokio::task::spawn_blocking(move || {
            let cf_handle = db
