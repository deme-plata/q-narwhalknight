...
impl StorageMetrics {
    ...
    pub async fn record_vertex_write(
        &self,
        latency: Duration,
        vertex_size: usize,
        payload_size: usize,
        ...
        debug!(
            "📊 Recording vertex write: {}ms, vertex: {}B, payload: {}B",
            latency.as_millis(),
            vertex_size,
            payload_size
        );
    ...
    pub async fn record_block_finalization(&self, latency: Duration, tx_count: usize) {
        debug!(
            "📊 Recording block finalization: {}ms, {} transactions",
            latency.as_millis(),
            tx_count
        );
    ...
    pub async fn get_current_metrics(&self) -> StorageMetricsSnapshot {
        ...
        let mut write_latencies: Vec<u64> =
            write_history.iter().map(|d| d.as_millis() as u64).collect();
        ...
        let mut read_latencies: Vec<u64> =
            read_history.iter().map(|d| d.as_millis() as u64).collect();
        read_latencies.sort();
...
mod tests {
    ...
    async fn test_metrics_tracking() {
        ...
        metrics
            .record_vertex_write(Duration::from_millis(50), 1024, 4096)
            .await;
        metrics
            .record_vertex_write(Duration::from_millis(75), 2048, 8192)
            .await;
    ...
    async fn test_performance_health() {
        ...
        for _ in 0..10 {
            metrics
                .record_vertex_write(Duration::from_millis(20), 1024, 2048)
                .await;
        ...
        for _ in 0..20 {
            metrics
                .record_vertex_write(Duration::from_millis(600), 1024, 2048)
                .await;
        }
    ...
    async fn test_throughput_calculation() {
        ...
        for _ in 0..100 {
            metrics
                .record_vertex_write(Duration::from_millis(10), 1024, 2048)
                .await;
        }
    ...
    async fn test_bandwidth_stats() {
        ...
        metrics
            .record_vertex_write(Duration::from_millis(10), 1024, 4096)
            .await; // 5KB total
