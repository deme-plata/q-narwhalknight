...
impl SnapshotManager {
    ...
    pub async fn create_snapshot(&self, block_height: u64) -> Result<SnapshotInfo> {
        ...
        self.save_snapshot_metadata(&snapshot_path, &snapshot_info)
            .await?;

        let duration = start_time.elapsed();
        info!(
            "✅ Created snapshot {} in {}ms (hot: {}MB, cold: {}MB)",
            snapshot_name,
            duration.as_millis(),
    ...
    async fn create_hot_checkpoint(&self, checkpoint_path: &Path) -> Result<()> {
        ...
        // Use fallback checkpoint method (compatible with all KVStore implementations)

        // Fallback: copy data manually (less efficient)
        warn!("📸 Using fallback checkpoint method for hot DB");
        self.manual_checkpoint(&self.hot_db, checkpoint_path, "hot")
            .await?;
    ...
    async fn create_cold_checkpoint(&self, checkpoint_path: &Path) -> Result<()> {
        ...
        self.manual_checkpoint(&self.cold_db, checkpoint_path, "cold")
            .await?;
    ...
    async fn manual_checkpoint(
        ...
        fs::write(
            marker_path,
            format!("Checkpoint created at {:?}", Instant::now()),
        )
        .await?;
    ...
    pub async fn get_snapshot_stats(&self) -> Result<SnapshotManagerStats> {
        ...
        let total_size: u64 = snapshots
            .iter()
            .map(|s| s.hot_db_size + s.cold_db_size)
            .sum();

        let latest_snapshot = snapshots.iter().max_by_key(|s| s.created_at).cloned();

        Ok(SnapshotManagerStats {
            total_snapshots: snapshots.len(),
            total_size_bytes: total_size,
            latest_snapshot,
            oldest_snapshot: snapshots.iter().min_by_key(|s| s.created_at).cloned(),
        })
    }
}
...
mod tests {
    ...
    async fn test_snapshot_manager_creation() {
        ...
        let hot_db = Arc::new(
            RocksDBKV::open_hot_db(temp_dir.path().join("hot"))
                .await
                .unwrap(),
        );
        let cold_db = Arc::new(
            RocksDBKV::open_cold_db(temp_dir.path().join("cold"))
                .await
                .unwrap(),
        );

        let manager = SnapshotManager::new(temp_dir.path().to_path_buf(), hot_db, cold_db).await;
    ...
    async fn test_snapshot_metadata_persistence() {
        ...
        let manager = SnapshotManager::new(temp_dir.path().to_path_buf(), hot_db, cold_db)
            .await
            .unwrap();
        ...
        manager
            .save_snapshot_metadata(&snapshot_info.path, &snapshot_info)
            .await
            .unwrap();

        let metadata_path = snapshot_info.path.join("snapshot_info.json");
        let loaded_info = manager
            .load_snapshot_metadata(&metadata_path)
            .await
            .unwrap();
