...
impl StorageManifest {
    ...
    pub async fn load_or_create(kv: &Arc<dyn KVStore>) -> Result<Self> {
        match kv.get(CF_MANIFEST, Self::MANIFEST_KEY).await? {
            Some(data) => {
                ...
                info!(
                    "📋 Loaded storage manifest - DAG round: {}, finalized: {}",
    ...
    pub async fn save(&self, kv: &Arc<dyn KVStore>) -> Result<()> {
        let data = bincode::serialize(self).context("Failed to serialize storage manifest")?;

        kv.put(CF_MANIFEST, Self::MANIFEST_KEY, &data)
            .await
    ...
    pub async fn update_dag_watermark(
        &mut self,
        kv: &Arc<dyn KVStore>,
        new_round: u64,
    ) -> Result<()> {
        if new_round > self.dag_round_watermark {
            debug!(
                "📈 Updating DAG watermark from {} to {}",
    ...
    pub async fn update_finalized_height(
        &mut self,
        kv: &Arc<dyn KVStore>,
        height: u64,
    ) -> Result<()> {
        if height > self.finalized_height {
            debug!(
                "📈 Updating finalized height from {} to {}",
    ...
    pub fn needs_recovery(&self) -> bool {
        ...
        let gap = self
            .finalized_height
            .saturating_sub(self.dag_round_watermark);
    ...
    pub fn validate(&self) -> Result<()> {
        if self.dag_round_watermark > self.finalized_height + 1000 {
            anyhow::bail!(
                "DAG watermark {} far ahead of finalized height {}",
                self.dag_round_watermark,
        ...
        if self.last_checkpoint_height > self.finalized_height {
            anyhow::bail!(
                "Checkpoint height {} ahead of finalized height {}",
    ...
    pub async fn upgrade_if_needed(&mut self, kv: &Arc<dyn KVStore>) -> Result<bool> {
        ...
        if self.version < CURRENT_VERSION {
            info!(
                "⬆️ Upgrading storage manifest from v{} to v{}",
