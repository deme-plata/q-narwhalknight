...
pub async fn create_hardware_rng(
    provider: HardwareProvider,
) -> Result<Box<dyn QRNGHardware + Send + Sync>> {
    match provider {
...
impl QRNGHardware for ThermalNoiseRNG {
    async fn generate_random_bytes(&self, count: usize) -> Result<Vec<u8>> {
        ...
        entropy_sources.extend_from_slice(
...
async fn connect_photonic_device(
    _device: &PhotonicDeviceInfo,
) -> Result<Box<dyn PhotonicDevice + Send + Sync>> {
    Err(anyhow::anyhow!(
        "Hardware photonic devices not available in Phase 0"
    ))
}
...
mod tests {
    ...
    async fn test_create_hardware_rng() {
        let rng = create_hardware_rng(HardwareProvider::Simulation)
            .await
            .unwrap();
