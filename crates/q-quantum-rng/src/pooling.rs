...
impl EntropyPool {
    ...
    pub fn add_entropy(&mut self, entropy: Vec<u8>, quality: f64) {
        debug!(
            "Adding {} bytes of entropy (quality: {:.3})",
            entropy.len(),
            quality
        );
    ...
    fn is_healthy(&self) -> bool {
        self.stats.average_quality > 0.8 && self.stats.current_fill_level > 0.1
    }
    ...
    pub fn quality_filter(&mut self, min_quality: f64) {
        ...
        while let (Some(score), Some(byte)) =
            (self.quality_scores.pop_front(), self.buffer.pop_front())
        {
    ...
    pub fn compact(&mut self, target_fill: f64) {
        ...
        debug!(
            "Compacted entropy pool to {:.1}% capacity",
            self.stats.current_fill_level * 100.0
        );
    }
}
