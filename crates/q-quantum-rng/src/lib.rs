...
pub use hardware::{HardwareProvider, QRNGHardware};
...
impl QuantumRNG {
    ...
    pub async fn generate_bytes(&self, count: usize) -> Result<Vec<u8>> {
        ...
        if self.config.enable_analysis {
            ...
            if quality.overall_score < self.config.min_entropy_quality {
                warn!(
                    "Entropy quality below threshold: {:.3} < {:.3}",
        ...
        self.update_generation_rate(bytes.len(), generation_time)
            .await;
        ...
        debug!(
            "Generated {} quantum random bytes in {:?}",
            bytes.len(),
            generation_time
        );
        Ok(bytes)
    ...
    async fn update_entropy_stats(&self, quality: f64) {
        ...
        stats.average_entropy_quality =
            (stats.average_entropy_quality * (total_samples - 1) as f64 + quality)
