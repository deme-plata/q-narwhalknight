...
fn block_frequency_test(bits: &[u8], block_size: usize, alpha: f64) -> Result<TestResult> {
    ...
    if num_blocks < 1 {
        return Err(anyhow::anyhow!(
            "Insufficient data for block frequency test"
        ));
    }
...
fn discrete_fourier_transform_test(bits: &[u8], alpha: f64) -> Result<TestResult> {
    ...
    let test_statistic =
        (peaks_above_threshold as f64 - expected_peaks) / (expected_peaks * 0.05).sqrt();
...
fn universal_statistical_test(bits: &[u8], alpha: f64) -> Result<TestResult> {
    ...
    if n < 387840 {
        return Err(anyhow::anyhow!(
            "Universal test requires at least 387840 bits"
        ));
    }
    ...
    let variance: f64 = 2.178; // Variance for l=7
...
fn approximate_entropy_test(bits: &[u8], alpha: f64) -> Result<TestResult> {
    ...
    if n < (1 << (m + 1)) {
        return Err(anyhow::anyhow!(
            "Insufficient data for approximate entropy test"
        ));
    }
...
fn quantum_coherence_test(bits: &[u8], alpha: f64) -> Result<TestResult> {
    ...
    if n < 1000 {
        return Err(anyhow::anyhow!(
            "Insufficient data for quantum coherence test"
        ));
    }
    ...
    let p_value = if average_correlation < 0.25 {
        0.95
    } else {
        0.05
    };
...
fn quantum_entanglement_test(bits: &[u8], alpha: f64) -> Result<TestResult> {
    ...
    if n < 2000 {
        return Err(anyhow::anyhow!(
            "Insufficient data for quantum entanglement test"
        ));
    }
