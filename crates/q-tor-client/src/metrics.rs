...
pub struct TorMetrics {
    ...
    // Dandelion++ metrics
    /// Transactions started in Dandelion++ protocol
    pub dandelion_transactions_started: Counter,
    /// Transactions received via Dandelion++
    pub dandelion_transactions_received: Counter,
    /// Stem phase forwards
    pub dandelion_stem_forwards: Counter,
    /// Stem to fluff transitions
    pub dandelion_stem_to_fluff: Counter,
    /// Fluff phase broadcasts
    pub dandelion_fluff_broadcasts: Counter,
    /// Normal (non-Dandelion++) transactions received
    pub normal_transactions_received: Counter,
}
...
impl TorMetrics {
    ...
    pub fn new() -> Self {
        Self {
            ...
            // Initialize Dandelion++ counters
            dandelion_transactions_started: Counter::new(),
            dandelion_transactions_received: Counter::new(),
            dandelion_stem_forwards: Counter::new(),
            dandelion_stem_to_fluff: Counter::new(),
            dandelion_fluff_broadcasts: Counter::new(),
            normal_transactions_received: Counter::new(),
        }
    }
    ...
    pub async fn record_connection_latency(&self, latency: Duration) {
        debug!(
            "📊 Recording Tor connection latency: {}ms",
            latency.as_millis()
        );
    ...
    pub async fn record_bytes_sent(&self, bytes: u64) {
        ...
        *total += bytes;
        ...
        debug!(
            "📤 Recorded {} bytes sent via Tor (total: {})",
            bytes, *total
        );
    }
    ...
    pub async fn record_bytes_received(&self, bytes: u64) {
        ...
        *total += bytes;
        ...
        debug!(
            "📥 Recorded {} bytes received via Tor (total: {})",
            bytes, *total
        );
    }
    ...
    pub async fn get_current_metrics(&self) -> TorMetricsSnapshot {
        ...
        let average_latency = if latency_history.is_empty() {
            ...
            let total_ms: u64 = latency_history.iter().map(|d| d.as_millis() as u64).sum();
            Duration::from_millis(total_ms / latency_history.len() as u64)
        };

        // Calculate 95th percentile latency
        let mut latencies: Vec<u64> = latency_history
            .iter()
...
impl TorHealthStatus {
    ...
    pub fn requires_action(&self) -> bool {
        matches!(
            self,
            Self::HighLatency | Self::CircuitIssues | Self::Offline
        )
    }
}
...
mod tests {
    ...
    async fn test_metrics_tracking() {
        ...
        metrics
            .record_connection_latency(Duration::from_millis(100))
            .await;
        metrics
            .record_connection_latency(Duration::from_millis(200))
            .await;
    ...
    async fn test_health_status() {
        ...
        for _ in 0..10 {
            metrics
                .record_connection_latency(Duration::from_millis(1500))
                .await;
        }
        let health = metrics.check_performance_health().await;
