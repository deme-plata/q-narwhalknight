...
const TopBar = memo(function TopBar({ currentBalance, nodeId, blockHeight, peers, isOnline, qci, onNavigate }: TopBarProps) {
  ...
  // v10.11.74: Freshness-gated balance reconciliation — single rule for ALL setters.
  // The displayed balance is written by four paths (currentBalance poll, qug-balance-changed,
  // dex-cooldown-expired, wallet-balance-updated). Two prior fixes each broke the other case:
  //   - last-write-wins  → lagging snapshot reads pulled mining gains back down (3.84<->3.993 flicker)
  //   - Math.max monotonic→ latched ANY transient high (stale cache / pending burst / fork phantom-mint
  //                          8M/1B) and never recovered (the 63k<->1.3M latch).
  // Fix: classify each write as an AUTHORITATIVE DELTA (an action THIS wallet performed — mining
  // credit, send, dex swap/deduct/cooldown) vs a SNAPSHOT read (currentBalance poll, generic SSE).
  // Authoritative deltas apply instantly and stamp a timestamp. Snapshots that DISAGREE are
  // suppressed only while a recent authoritative delta is still settling; after the window any
  // snapshot wins, so a genuine correction (up OR down) always lands and nothing latches forever.
  const lastAuthoritativeTsRef = useRef<number>(0);
  const FRESHNESS_WINDOW_MS = 4000;

  const applyAuthoritativeBalance = (value: number) => {
    setStableBalance(value);
    lastBalanceUpdateRef.current = Date.now();
    lastAuthoritativeTsRef.current = Date.now();
  };

  const applySnapshotBalance = (value: number) => {
    const sinceAuthoritative = Date.now() - lastAuthoritativeTsRef.current;
    if (sinceAuthoritative < FRESHNESS_WINDOW_MS) {
      const stable = stableBalanceRef.current;
      // Within the settle window a snapshot that merely disagrees with the just-applied
      // authoritative value is lag — ignore it. (A matching value is a harmless no-op.)
      if (isValidBalance(stable) && Math.abs(value - stable) > 0.0001) {
        console.log(`⏳ TopBar: suppressing lagging snapshot ${value} (authoritative ${stable}, ${sinceAuthoritative}ms ago)`);
        return;
      }
    }
    setStableBalance(value);
  ...
  useEffect(() => {
    ...
    if (balanceDifference > 1 || timeSinceLastUpdate > balanceStabilityWindowMs) {
      ...
      if (Math.abs(bestBalance - currentStable) > 0.0001) {
        ...
        // v10.11.74: snapshot read (polled currentBalance) — freshness-gated so a lagging
        // poll cannot pull a freshly-credited mining/tx balance back down.
        applySnapshotBalance(bestBalance);
      }
    }
  }, [currentBalance]);
  ...
  useEffect(() => {
    const handleBalanceChanged = (event: Event) => {
      ...
      // v10.11.74: authoritative DEX action — apply directly + stamp freshness.
      applyAuthoritativeBalance(newBalance);
    };
    ...
    const handleDexCooldownExpired = (event: Event) => {
      ...
      // v10.11.74: authoritative DEX cooldown re-read — apply directly + stamp freshness.
      applyAuthoritativeBalance(qugBalance);
    };
    ...
    const handleWalletBalanceUpdated = (event: Event) => {
      ...
      if (isMiningUpdate || isDexSwapDeduct || isTransactionSent) {
        ...
        // v10.11.74: authoritative delta (this wallet's own mining/tx/dex action) — apply + stamp.
        applyAuthoritativeBalance(balance);
      } else {
        // v10.11.74: generic balance-updated with no action reason = a snapshot read.
        // Freshness-gated so it can't bounce a freshly-credited balance back to a lagging value,
        // but is NOT a Math.max latch — after the settle window any correction (up or down) lands.
        applySnapshotBalance(balance);
      }
    };
