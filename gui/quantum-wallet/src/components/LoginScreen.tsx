...
export default function LoginScreen({ onAuthenticate }: LoginScreenProps) {
  ...
  // Validate seed phrase against the BIP39 wordlist + checksum.
  //
  // 2026-05-17 SECURITY FIX (brainwallet incident): the previous version
  // only counted words (12 or 24) and word length (3-8 chars). That let
  // ANY 12 made-up 3-letter words pass, and the downstream keypair
  // derivation just SHA3-256 hashed the string — so common phrases like
  // "test test test..." or "abc def ghi..." deterministically derived to
  // PUBLIC well-known addresses that multiple users shared. Reported by
  // a community member who accidentally landed in someone else's wallet.
  // The new check uses @scure/bip39 validateMnemonic which enforces both
  // the wordlist and the BIP39 checksum (only ~1 in 16 random 12-word
  // combos pass; real BIP39-generated phrases always pass).
  const validateSeedPhrase = (phrase: string): { valid: boolean; error?: string } => {
    const normalized = phrase.trim().toLowerCase().split(/\s+/).filter(w => w.length > 0);
    ...
    if (normalized.length !== 12 && normalized.length !== 24) {
      return { valid: false, error: `Recovery phrase must be exactly 12 or 24 words (got ${normalized.length} words)` };
    }

    // @scure/bip39 validates wordlist membership + checksum. This is the
    // gate that prevents brainwallet collisions. Do not loosen.
    if (!bip39ValidateMnemonic(normalized.join(' '), bip39English)) {
      return {
        valid: false,
        error:
          'Recovery phrase is not a valid BIP39 mnemonic. Each word must be ' +
          'from the BIP39 English wordlist and the phrase must include a ' +
          'matching checksum. Use a phrase generated by this wallet (or any ' +
          'BIP39-compliant wallet) — random words or numbers will not work.',
      };
    }
          ...
          >
            ...
            >
              ...
              <div className="space-y-3">
                {/* Linux x64 */}
                <a
                  href="https://quillon.xyz/downloads/q-api-server-v10.3.8"
                ...
                {/* Windows x64 */}
                <a
                  href="https://quillon.xyz/downloads/q-api-server-v10.3.8-windows-x64.exe"
...
{`wget https://quillon.xyz/downloads/q-api-server-v10.3.8
