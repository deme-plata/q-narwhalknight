...
import QuillonWelcomeModal, { QUILLON_WELCOME_STORAGE_KEY } from './QuillonWelcomeModal';
...
const Dashboard = memo(function Dashboard({ onNavigateToSend, liveBalance, onNavigateToChat }: DashboardProps) {
  ...
  // v10.11.75-FE: Quillon Graph welcome modal — replaces Stability Horizon.
  // Auto-opens once (per QUILLON_WELCOME_STORAGE_KEY) after a short beat.
  useEffect(() => {
    if (localStorage.getItem(QUILLON_WELCOME_STORAGE_KEY)) return;
    const timer = setTimeout(() => setShowWelcomeModal(true), 1800);
...
Transactions (recent): ${recentTransactions.slice(0, 10).length}`;
    ...
    const fetchNodeStatusCore = async () => {
      ...
      try {
          ...
          if (currentWalletAddress) {
            ...
            try {
              ...
              if (balanceResponse.success && balanceResponse.data) {
                ...
                walletBalance = previousHighest > 0 ? previousHighest : cachedValue; // v10.11.65: last-known, not all-time-max (stops inflated latch)
        ...
        if (mounted) {
          ...
          const cachedValue = cachedBalance ? parseFloat(cachedBalance) : 0;
          const previousHighest = highestKnownBalancesRef.current['QUG'] || 0;
          const bestBalance = (previousHighest > 0 ? previousHighest : cachedValue) /* v10.11.74: prefer latest-known, not stale-high-cache — stops post-send flicker */;
  ...
  useEffect(() => {
    const handleQugBalanceChanged = (event: Event) => {
      ...
      if (typeof newBalance === 'number') {
        ...
        }, 15000); // v10.11.74: align with the 15s dexCooldownUntil window (TopBar/DexScreen) — was 10s, the 5s gap let old/new flicker
  ...
  useEffect(() => {
    if (refreshTrigger > 0) {
      ...
      const refresh = async () => {
        ...
        const qugBalance = nodeBalance > 0 ? nodeBalance : (previousHighest > 0 ? previousHighest : cachedValue); // v10.11.65: trust authoritative/last-known, not all-time-max
  ...
  return (
    <div className="space-y-8 relative">
      ...
      {showWelcomeModal && (
        <QuillonWelcomeModal onClose={() => setShowWelcomeModal(false)} />
      )}
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1" style={{ scrollbarWidth: 'none' }}>
            <AnimatePresence initial={false}>
              {paginatedTransactions.length > 0 ? paginatedTransactions.map((tx) => {
                ...
                const accentColor = tx.type === 'receive' ? '#22C55E'
                ...
                {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                  ...
                  return (
                    <motion.button key={p} onClick={() => setCurrentPage(p)}
                      ...
                      style={currentPage === p ? { background: 'linear-gradient(135deg,#D4AF37,#F59E0B)', color: '#0a0c14' } : { background: 'rgba(255,255,255,0.04)', color: 'rgba(255,255,255,0.3)' }}
                      whileTap={{ scale: 0.9 }}>
                      {p}
                    </motion.button>
                  );
