...
// FIX 2026-06-29: @noble/ed25519 v2.3.0 made sign()/getPublicKey() SYNC and dropped the
// implicit sha512 default. Without an explicit hook they throw "sha512Sync is not a
// function" (minified "S is not a function"), breaking ALL wallet sends. Set once at load.
{
  const _ed: any = ed25519 as any;
  if (_ed && _ed.etc && !_ed.etc.sha512Sync) {
    _ed.etc.sha512Sync = (...m: Uint8Array[]) => _nobleSha512(_ed.etc.concatBytes(...m));
  }
  if (_ed && _ed.etc && !_ed.etc.sha512Async) {
    _ed.etc.sha512Async = (...m: Uint8Array[]) => Promise.resolve(_nobleSha512(_ed.etc.concatBytes(...m)));
  }
}
import { entropyToMnemonic as _entropyToMnemonic, validateMnemonic } from '@scure/bip39';
 ...
 *
 * 2026-05-17: BIP39 validation MANDATORY. Without it, any string SHA3-hashes
 * to a deterministic Ed25519 key — so common phrases like "test test test..."
 * land multiple users in the SAME wallet ("brainwallet" vulnerability).
 * `@scure/bip39`'s `validateMnemonic` checks both the wordlist membership
 * AND the BIP39 checksum; only ~1 in 16 random 12-word combos pass, and
 * properly-derived mnemonics always pass.
 *
 * Throws if the input is not a valid BIP39 mnemonic. Callers should display
...
export async function keypairFromMnemonic(mnemonic: string): Promise<WalletKeyPair> {
  ...
  if (!validateMnemonic(normalized, english)) {
    throw new Error(
      'Invalid mnemonic: words must be from the BIP39 wordlist and the ' +
      'checksum must match. Use a recovery phrase generated by this wallet ' +
      '(or any BIP39-compliant wallet). Random words / numbers will not work.'
    );
  }
  // Hash mnemonic to get private key (same as backend implementation)
  // Preserves backward compatibility: existing valid-BIP39 wallets still
  // derive to the same address. The new validation only rejects junk input.
...
export interface SignedTransferV72 {
  ...
  nonce: number;     // nonce signed over
  timestamp: number; // unix seconds signed over
  fee: string;       // base-unit fee signed over (decimal string)
}

/**
 * Build the canonical p2p signable payload and Ed25519-sign its SHA3-256 hash.
 * `fromHex`/`toHex` are 64-char hex (no `qnk` prefix). `amountBase` is the u128
 * base-24 amount as a bigint. Returns the fields the server needs to rebuild +
 * verify the exact same bytes.
 */
export async function signTransferV72(
  privateKey: Uint8Array,
  fromHex: string,
  toHex: string,
  amountBase: bigint,
  ...
  feeBase: bigint = MIN_TRANSACTION_FEE_BASE,
  tokenType: string = 'QUG',
  data: Uint8Array | string = new Uint8Array(0)
): Promise<SignedTransferV72> {
  ...
  // v10.11.74: api.ts passes the resolved token address as a HEX STRING (empty for QUG).
  const dataBytes: Uint8Array = typeof data === "string"
    ? (data.length ? hexToBytes(data) : new Uint8Array(0))
    : data;
  const tu = tokenType.toUpperCase();
  const tokenByte =
  ...
  const parts: Uint8Array[] = [
    ...
    from,
    to,
    leBytes(amountBase, 16),
    leBytes(feeBase, 16),
    leBytes(BigInt(nonce), 8),
    leBytes(BigInt(timestampSecs), 8),
    new Uint8Array([tokenByte]),
    leBytes(BigInt(dataBytes.length), 4),
    dataBytes,
  ];
  let total = 0;
  ...
  const digest = sha3_256(payload); // 32-byte SHA3-256 — the message the verifier checks
