...
use q_quantum_mixing::{
    QuantumMixingPlugin, QuantumMixingConfig, InitiateMixRequest, UserMixingPreferences,
    TransactionInfo, MixingPoolType, PrivacyLevel
};
...
async fn main() -> anyhow::Result<()> {
    ...
    match quantum_mixer.generate_stark_mixing_proof("test_session_stark", &test_transactions).await {
        Ok(proof) => {
    ...
    match quantum_mixer.generate_batch_stark_proof(&batch_sessions).await {
        Ok(_bundle) => {
    ...
    match quantum_mixer.calculate_privacy_metrics("test_session").await {
        Ok(metrics) => {
    ...
    println!("   STARK proofs enabled: {}", mixing_config.enable_stark_proofs);
    println!("   GPU acceleration: {}", mixing_config.stark_gpu_acceleration);
    println!("   Security level: {} bits", mixing_config.security_level);
    println!("   Quantum entropy sources: {:?}", mixing_config.quantum_entropy_sources);
    println!("   Mixing duration: {}-{} seconds", 
             mixing_config.min_mixing_duration_seconds,
             mixing_config.max_mixing_duration_seconds);
    ...
    println!("   5 mixing sessions initiated in: {:?}", benchmark_duration);
