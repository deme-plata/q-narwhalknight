...
\begin{itemize}[nosep]
  ...
  \item Ready for production testing on Epsilon (48 cores, 10\,Gbit NIC).
\end{itemize}
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Metric} & \textbf{Target} & \textbf{Design Choice} \\
\midrule
RPS (48 cores)         & $>$500K      & Worker-per-core, \texttt{SO\_REUSEPORT} \\
P99 latency            & $<$5\,ms     & SIMD parsing, connection pooling \\
Memory (idle)          & $<$50\,MB    & Lock-free atomics, no GC \\
Memory (100K conns)    & $<$500\,MB   & Semaphore backpressure \\
TLS handshakes/sec     & $>$50K       & 1M session cache, session tickets \\
Static file memory     & 64\,KB/download & Streaming (was 100\,MB/download) \\
CRC32 throughput       & $>$1\,GB/s   & Hardware CRC32C via \texttt{crc32fast} \\
\bottomrule
\end{tabular}
\end{table}
...
Q-NarwhalKnight workload (9,600 req/s, 400+ miners).}
\label{tab:comparison}
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Metric} & \textbf{nginx} & \textbf{Caddy} & \textbf{q-flux (target)} \\
\midrule
Architecture        & Process pool    & Goroutine pool  & Worker-per-core \\
Connection model    & Per-request TCP & Per-request TCP  & Keep-alive pool \\
CPU at 9.6K RPS     & 4,200\%         & 3,785\%         & $<$100\% \\
Memory at 9.6K RPS  & 2\,GB           & 11.5\,GB        & $<$500\,MB \\
503 error rate      & 87\%            & 78\%            & 0\% \\
Max concurrent conns & 75K (leaked)   & 88K goroutines  & 100K (bounded) \\
TLS library         & OpenSSL         & Go TLS          & rustls \\
Configuration       & Text config     & Caddyfile       & TOML \\
\bottomrule
\end{tabular}
\end{table}
...
The system is ready for production testing on Epsilon---the 48-core,
10\,Gbit supernode that anchors the Q-NarwhalKnight mainnet.
Deployment will follow a staged rollout: 10\% traffic split with
Caddy, gradual ramp to 100\%, and finally io\_uring activation on
