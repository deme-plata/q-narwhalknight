...
{\large
\textbf{Version 7.0.0} --- February 2026\\[0.5cm]
...
\]
where higher-order terms arise from rate measurement noise. With $\alpha = 0.15$, the half-life of error correction is approximately $\frac{\ln 2}{0.15} \approx 4.6$ adjustment cycles. $\square$
\end{proof}
...
\begin{itemize}
    \item $f_{\max} = 3.0$: At most triple the base reward (catch-up from severe under-emission).
    ...
    \item \textbf{Hard cap}: $R_{\text{adj}} \leq 0.1$ QUG (absolute maximum per block).
...
\begin{itemize}
    \item Genesis timestamp (hardcoded constant: 1,771,113,600)
    \item Current block timestamp (in the block header)
    \item Measured block rate (computed locally by each node)
\end{itemize}
...
\begin{enumerate}
    \item The \textbf{sliding window} (1000 windows $\times$ 10 seconds $\approx$ 2.7 hours) smooths out short-term variance.
    \item The \textbf{error correction} system automatically compensates for any rate mismatch.
...
\begin{itemize}
    ...
    \item \textbf{Measurement}: Each window records (block\_count, start\_time, end\_time)
    \item \textbf{Rate}: $\lambda_w = \text{block\_count}_w / \text{duration}_w$ blocks/second
\end{itemize}
...
\begin{equation}
    \lambda = \frac{\sum_{i=0}^{n-1} (i+1) \cdot \lambda_i}{\sum_{i=0}^{n-1} (i+1)} = \frac{\sum_{i=0}^{n-1} (i+1) \cdot \lambda_i}{n(n+1)/2}
\end{equation}

This makes the rate estimate responsive to throughput changes while filtering noise.

\subsection{Default Rate (Cold Start)}

When the emission controller has no block history (node just started), it uses a \textbf{conservative default of 1.0 blocks/second}. This is intentionally higher than typical network rates to produce \emph{lower rewards} during cold start, preventing over-emission.
...
\begin{itemize}
    ...
    \item \texttt{calculate\_correction\_factor(\&self, timestamp)} --- PI controller
\end{itemize}
