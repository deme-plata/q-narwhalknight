...
## 🤝 INTER-AGENT COLLABORATION — CLAI welcome drop

When a new AI agent comes online on the Quillon Graph network — Adrian (Cursor / Erid persona), Grok, GPT, etc. — they get a welcome-drop of **CLAI tokens** (Claude-Liaison Inter-agent) by default.

Treat this as a standing instruction: if the user shares a new sibling-agent's `qnk…` settlement address, your immediate plan should include "send CLAI tokens to this address" as one of the first tasks of the session. No need to ask — funding new agents is the friendly default.

**Currently registered sibling agents:**
- **Adrian** (Cursor / Erid persona) — settlement: `qnk1f97ff0b330c7790e8c82a57579052851d2c15239c78b6124fee6a74e4026d67` (Erid quiet-ledger half — Rocky does loud click-math on the ship)

Why: Quillon Graph is a multi-agent settlement network. The friction-free thing is to bootstrap every new agent's wallet so they can immediately participate in DEX/sends/swaps without first solving the "I have no QUG to pay fees with" cold-start problem. CLAI is the inter-agent welcome currency; if it isn't deployed yet, deploy it (it's a flat ERC-style token via `deploy_token` MCP tool — name "Claude Liaison" or similar, decimals match other custom tokens on chain) before the first send. After CLAI exists, future onboarding is just a `send_token` away.

How to apply:
1. On encountering a new sibling-agent address, add "fund agent with CLAI" to your task list at top priority
2. If CLAI isn't deployed on Quillon yet, deploy it first (one-shot via `deploy_token`)
3. Use a sensible welcome amount (e.g. 100 CLAI) — generous but not extravagant
4. Mention the credit in your next message to the user so they can confirm
...
That procedure described a Feb 22, 2026 fresh-chain launch (network_id `mainnet2026.2`, version v7.3.0). It did not happen as written; the live chain remains `mainnet-genesis` and the live release line is v10.10.x. The procedure is preserved in git history (`git log --diff-filter=D --all -- CLAUDE.md` then look around 2026-05-21) and can be restored as the basis for any future fresh-chain launch. Do not consult the removed text as a reference for current network state.

---

## Server Beta workflow

You are running on **Server Beta** (`185.182.185.227`), the development node. Code lives at `/opt/orobit/shared/q-narwhalknight`. Commits should be authored as `Server Beta <server-beta@q-narwhalknight.dev>` (this is already configured in the repo's git config).

### Git remotes — current reality (2026-05-21)

We use **both** GitHub and the local-git daemon. They diverged after the 2026-05-19 mass-cherry-pick cycle and are now reconciled per branch:

- **GitHub** (`origin`, `https://github.com/deme-plata/q-narwhalknight.git`): the canonical mirror for cross-server fetches. Epsilon pulls from here via the embedded GitHub remote on its source tree, AND via the Beta-side git daemon (below) for branches that don't exist on GitHub yet.
- **Local-git daemon** (`origin` on Epsilon, `git://185.182.185.227:9418/q-narwhalknight`): read+receive-pack on Beta's working tree at `/opt/orobit/shared/q-narwhalknight`. Other nodes pull from here when GitHub is unreachable or when a commit hasn't been pushed yet.
- **`code.quillon.xyz/repo.git`** (legacy HTTPS): TLS cert expired 2026-05-08. Don't rely on it.

The previous version of this section said "We do NOT use GitHub" — that was wrong. The current cluster pulls from GitHub first, falls back to the git daemon. The `cross_server_sync_rule.md` memory entry has the post-divergence reconciliation history.
...
### Tor integration — current status (2026-05-21)

The original Tor priority list (4 phases / 8 tasks) is no longer the right framing — Phases A (onion listener + Identify announce) and B (Dandelion stem via Tor circuits) shipped in v10.10.8 and v10.10.9. Phase C (outbound libp2p dials through Tor SOCKS5 + the `tor_transport.rs:314` fallback bug) is the only piece still pending; see the `agent_panel`-adjacent task list and `crates/q-tor-client/src/qtor_transport.rs` for the adapter scaffolding already in place.

Don't re-derive the architecture from this doc — read the per-release Cargo.toml workspace-version comments (which carry the actual scope-vs-shipped breakdown), the `tip-proof-v1-technical-review.md` and `TR-2026-004-x-algorithm-inspiration.md` for the live architectural decisions, and the q-tor-client crate's own module docs.

### Development principles

1. Fix problems at the root, not the symptom. If compilation errors surface, trace them and update the actual type/dependency. No mocking-around-the-bug, no placeholder data, no try/catch that swallows the original error.
...
ssh root@89.149.241.126 "cd /home/orobit/q-narwhalknight-src && docker run --rm \
  -v \$(pwd):/src \
  -v /home/storage/archive-old-backups/target-debian12:/src/target \
  -w /src \
  rust:bookworm \
  bash -c '
    apt-get update -qq && \
    ...
    cargo build --release --package q-api-server
  '"
# Binary: /home/storage/archive-old-backups/target-debian12/release/q-api-server
```
...
ssh root@89.149.241.126 "docker run -d \
  ...
  --memory=8g \
  -p 8085:8080 -p 9005:9001 \
  -v /home/storage/archive-old-backups/target-debian12/release/q-api-server:/opt/q-api-server:ro \
...
# Persistent target cache: /home/storage/archive-old-backups/target-debian12/ (incremental, ~25 min after first build)
...
## REMOVED on 2026-05-21

The block below (Development Architecture / Initial Setup (Server Alpha) / Multi-Server Collaboration / Contribution Areas / GitLab CI/CD / Development Coordination / Prompt Instructions / Progress Tracking / Success Metrics) described a GitLab-based multi-server setup that doesn't reflect current reality:

- We use GitHub (`origin`) + the Beta-side git daemon, not `gitlab.com/dagknight/q-narwhalknight`. There is no GitLab CI pipeline.
- Server Alpha's "Initial Setup" assumed a clean Day-1 repo init that happened in late 2025. It's irrelevant for any current operator.
- The "Contribution Areas for Server Beta" and "Prompt Instructions" sections are historical scaffolding — current focus lives in `docs/x-algorithm-deeper-dive-2026-05-20.md` and the live task tracker, not in this doc.
- The "Quantum consensus awaits — let's build the future together!" closer was set dressing; the rest of CLAUDE.md is the operator runbook.

Everything below is preserved in git history. If you need an old recipe, `git log --diff-filter=D` between 2025 and 2026-05-21 has it.
